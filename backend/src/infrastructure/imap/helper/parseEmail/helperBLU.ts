...
export function extractInfoFromHTMLBLU(html: string): ExtractedInfo {
  ...
  const labelDate = $('div').filter(
    (i, el) => $(el).text().trim() === 'Tgl & Jam Transaksi',
  );
  const dateTimeRaw = labelDate
    .closest('div[class*="mj-column"]')
    .next('div[class*="mj-column"]')
    .text()
    .trim();

  const labelTotal = $('div').filter(
    (i, el) => $(el).text().trim() === 'Total',
  );
  let totalAmountRaw = labelTotal
    .closest('div[class*="mj-column"]')
    .next('div[class*="mj-column"]')
    .text()
    .trim();
