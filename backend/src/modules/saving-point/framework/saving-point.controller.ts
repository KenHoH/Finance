...
export class SavingPointController {
  ...
  async allocateToGoal(
    @Req() req: Request,
    @Param('id') id: string,
    @Body() dto: AllocateToGoalDto,
  ) {
    if (!req.user) throw new NotFoundException('not authenticated');
    try {
      return await this.savingPointService.allocateToGoal(
        req.user.sub,
        id,
        dto,
      );
  ...
  async allocateToInvestment(
    @Req() req: Request,
    @Param('id') id: string,
    @Body() dto: AllocateToInvestmentDto,
  ) {
    if (!req.user) throw new NotFoundException('not authenticated');
    try {
      return await this.savingPointService.allocateToInvestment(
        req.user.sub,
        id,
        dto,
      );
