...
export class AuthService {
  ...
  async handleGoogleLogin(code: string, state?: string) {
    ...
    const avatarUrl =
      (profile as unknown as Record<string, string | undefined>).picture ||
      null;
  ...
  buildFrontendRedirect(state?: string) {
    ...
    try {
      ...
      const redirectUrl = returnTo.startsWith('/')
        ? new URL(returnTo, fallbackUrl)
        : new URL(returnTo);
      if (
        !this.isAllowedFrontendOrigin(redirectUrl.origin, fallbackUrl.origin)
      ) {
        return fallback;
      }
  ...
  async getMe(token: string): Promise<{
    ...
    try {
      ...
      if (cached && cached.expires > now) {
        return { user: cached.user, isInvalid: false };
      }
      ...
      this.userCache.set(payload.sub, {
        user: result,
        expires: now + this.CACHE_TTL_MS,
      });
      return { user: result, isInvalid: false };
    } catch {
  ...
  private getHmacSecret(): string {
    return (
      this.configService.get<string>('JWT_SECRET') || 'fallback-hmac-secret'
    );
  }

  private signPayload(payload: string): string {
    return createHmac('sha256', this.getHmacSecret())
      .update(payload)
      .digest('hex');
  }
  ...
  private encodeOauthState(returnTo?: string) {
    ...
    if (returnTo.startsWith('/')) {
      const p1 = Buffer.from(JSON.stringify({ returnTo }), 'utf8').toString(
        'base64url',
      );
    ...
    try {
      ...
      const p2 = Buffer.from(
        JSON.stringify({ returnTo: url.toString() }),
        'utf8',
      ).toString('base64url');
