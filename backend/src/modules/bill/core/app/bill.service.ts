...
export class BillService {
  ...
  async create(userId: string, dto: CreateBillDto) {
    ...
    await this.activityLogService.logActivity(
      ...
      'Bill',
      bill.id,
      { title: bill.title, amount: Number(bill.amount) },
    );

    if (bill.isReminderEnabled && !bill.paidAt) {
      const cat = bill.categoryId
  ...
  async update(userId: string, id: string, dto: UpdateBillDto) {
    ...
    await this.activityLogService.logActivity(userId, 'UPDATE', 'Bill', id, {
      title: updated.title,
    ...
    if (
      ...
      (updated.status === 'PENDING' || updated.status === 'OVERDUE')
    ) {
  ...
  async payBill(userId: string, id: string, dto: PayBillDto) {
    ...
    await this.activityLogService.logActivity(userId, 'PAY', 'Bill', id, {
      title: paid.title,
