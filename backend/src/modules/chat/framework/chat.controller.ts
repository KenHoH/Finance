...
export class ChatController {
  ...
  async chat(@Body() dto: ChatRequestDto, @Res() res: Response){
    console.log('[ChatController] Received request, dto:', typeof dto, dto ? Object.keys(dto) : 'null');

    try{
      if(!dto || !dto.messages || !Array.isArray(dto.messages) || dto.messages.length === 0){
        console.log('[ChatController] Validation failed: missing messages');
        res.status(HttpStatus.BAD_REQUEST).json({ error: 'Messages are required' });
        return;
      }
      if(!dto.model){
      ...
      console.log('[ChatController] Calling chatService with model:', dto.model);
      const backendRes = await this.chatService.chat(dto.messages, dto.model);
      console.log('[ChatController] OpenRouter response status:', backendRes.status, 'ok:', backendRes.ok);

      if(!backendRes.ok){
        const errBody = await backendRes.text().catch(() => 'OpenRouter error');
        console.error('[ChatController] OpenRouter error:', backendRes.status, errBody);
      ...
      if(!backendRes.body){
        ...
        res.status(HttpStatus.BAD_GATEWAY).json({ error: 'No response from AI provider' });
        return;
      }
      ...
      console.error('[ChatController] Caught error:', message, err instanceof Error ? err.stack : '');
      if(!res.headersSent){
