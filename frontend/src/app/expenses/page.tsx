            ...
            <div className="h-[200px] w-full">
              <ResponsiveContainer>
                <PieChart>
                  ...
                  <RechartsTooltip formatter={(val) => formatCurrency(Number(val))} />
        ...
        <div className="h-[250px] w-full">
          <ResponsiveContainer>
            <BarChart data={trendData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <RechartsTooltip cursor={{ fill: 'var(--accent)' }} formatter={(val) => formatCurrency(Number(val))} />
            ...
            <tbody className="divide-y divide-border">
              {paginatedData.map((t) => (
                <tr key={t.id} onClick={() => setSelectedTx(t)} className="hover:bg-muted/50 transition-colors cursor-pointer group">
                  ...
                  <td className="px-6 py-4 text-right font-bold text-rose-500">-{formatCurrency(t.amount)}</td>
                </tr>
              ))}
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[95vw] max-w-md bg-card rounded-3xl shadow-2xl z-50 p-6">
              ...
              <div className="text-center mb-8">
                <h3 className="text-4xl font-extrabold text-rose-500 mb-2">-{formatCurrency(selectedTx.amount)}</h3>
