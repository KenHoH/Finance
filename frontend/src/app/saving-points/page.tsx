...
export default function SavingPointsPage() {
  ...
  const { data: categories = [] } = useQuery<Category[]>({
    ...
    queryFn: async () => {
      ...
      return Array.isArray(res)
        ? res.filter((c: Category) => c.type === "INVESTMENT")
        : [];
    },
  });
  ...
  const allocateGoalMutation = useMutation({
    ...
    onMutate: async (dto) => {
      ...
      await optimisticUpdate(queryClient, ["saving-points"], dto.id, {
        savingAmount: Math.max(0, prevAmount - dto.amount),
      });
      ...
      await optimisticUpdate(queryClient, ["goals"], dto.goalId, {
        currentAmount: prevCurrent + dto.amount,
      });
      return {};
    },
    onError: (err) => {
  ...
  const allocateInvestmentMutation = useMutation({
    ...
    onError: (err) => {
      addToast(
        extractApiError(err, "Failed to allocate to investment"),
        "error",
      );
    },
    onSettled: () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-24">
          ...
          >
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                onClick={() => {
                  setEditPoint(point);
                  setEditAmount(String(point.savingAmount));
                }}
      ...
      <Modal
        ...
        description={
          allocateModal
            ? `Available: ${formatCurrency(allocateModal.amount)}`
            : ""
        }
      >
        {/* Tabs */}
              ...
              >
                ...
                {goals.map((g) => (
                  <option key={g.id} value={g.id}>
                    {g.name} ({formatCurrency(Number(g.currentAmount))} /{" "}
          ...
          {allocateTab === "investment" && (
            <div>
              ...
              <select
                ...
                onChange={(e) => setInvestCategoryId(e.target.value)}
                className="w-full bg-background border border-border rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-primary"
              >
          ...
          {allocateTab === "debt" && (
            <div>
              ...
              <select
                id="debt-select"
                value={debtPointId}
                onChange={(e) => setDebtPointId(e.target.value)}
                className="w-full bg-background border border-border rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-primary"
              >
                <option value="">Select debt</option>
                {debtPoints.map((d) => (
                  <option key={d.id} value={d.id}>
                    {d.budget?.category?.name || "Budget"} —{" "}
                    {formatCurrency(Number(d.debtAmount))}
                  </option>
                ))}
              </select>
              {debtPoints.length === 0 && (
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button
            onClick={() => {
              ...
              if (
                allocateModal &&
                Number(allocateAmount) > allocateModal.amount
              ) {
              ...
              if (allocateTab === "goal") {
                ...
                allocateInvestmentMutation.mutate({
                  ...dto,
            ...
            disabled={
              ...
              allocateInvestmentMutation.isPending ||
      ...
      >
        ...
        <div className="flex justify-end gap-2 mt-5">
          ...
          <button
            onClick={() => {
              const err = validateNumber(editAmount, "Saving Amount", {
                min: 0.01,
              });
              if (err) {
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (pointToDelete) deleteMutation.mutate(pointToDelete);
        }}
