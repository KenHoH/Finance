    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 space-y-8 max-w-7xl mx-auto pb-24 relative">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-indigo-500 text-white px-5 py-2.5 rounded-xl font-semibold shadow-md btn-ripple hover:shadow-lg transition-all active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-border glass-panel hover-pop p-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-3xl border border-border glass-panel hover-pop p-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-3xl border border-border glass-panel hover-pop p-6">
      ...
      {/* Tabs */}
      <div className="flex glass-panel rounded-2xl p-1.5 w-fit shadow-sm">
        {(['all', 'i_owe', 'they_owe'] as const).map((tab) => (
          <button
            ...
            className={cn(
              ...
              activeTab === tab ? "bg-indigo-500 text-white shadow-md btn-ripple" : "text-muted-foreground hover:bg-accent/50"
            )}
          >
        ...
        <AnimatePresence mode="popLayout">
          {filteredBills.map((bill, idx) => (
            <motion.div 
              ...
              className="group glass-panel hover-pop rounded-3xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 cursor-pointer"
            >
          ...
          <div className="space-y-6">
            <div className="bg-accent/50 rounded-2xl p-4 flex justify-between items-center">
              ...
              <div className="text-right">
                ...
                <p className="text-2xl font-extrabold text-primary">{formatCurrency(selectedBillDetail.totalAmount).replace(',00', '')}</p>
              </div>
            </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                {selectedBillDetail.participants.map((p: any, i: number) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-accent/50 transition-colors">
                    ...
                    <div className="text-right">
                      <p className="text-base font-bold text-foreground">{formatCurrency(p.amount).replace(',00', '')}</p>
                      {p.status === 'paid' ? (
