...
import {
  PieChart, Plus, Edit2, AlertTriangle, CheckCircle2, AlertCircle, Eye, Receipt, icons
} from "lucide-react";
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 space-y-8 max-w-7xl mx-auto pb-24">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-amber-500 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-amber-500/20 transition-all active:scale-95 hover:bg-amber-600"
        >
          ...
          {selectedBudget && generateDummyTransactions(selectedBudget.id, selectedBudget.category).map((tx) => (
            <div key={tx.id} className="flex items-center justify-between p-4 rounded-2xl bg-accent/50 border border-border/50 hover:bg-accent transition-colors">
              ...
              <div className="text-right">
                <p className="font-bold text-rose-500">-{formatCurrency(tx.amount)}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-border p-8">
          <p className="text-sm font-bold text-muted-foreground uppercase tracking-wider mb-2">Total Budgeted</p>
          <p className="text-4xl font-extrabold text-foreground">{formatCurrency(totalBudgeted)}</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-3xl border border-border p-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-3xl border border-border p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {budgetsData.map((budget, idx) => {
          ...
          return (
            <motion.div key={budget.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * idx }} className="rounded-[2rem] border border-border p-6 flex flex-col justify-between relative group overflow-hidden">
              ...
              <div className="mb-8">
                <div className="flex justify-between items-end mb-3">
                  <span className={cn("text-3xl font-black tracking-tight", statusText)}>{formatCurrency(budget.spent)}</span>
                  <span className="text-sm font-bold text-muted-foreground">of {formatCurrency(budget.limit)}</span>
                </div>
              ...
              <button 
                ...
                className="w-full py-3.5 bg-background hover:bg-accent border border-border text-foreground font-bold rounded-2xl flex items-center justify-center gap-2 transition-all hover:shadow-md"
              >
