...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      useAuthStore.getState().login({ id: "1", name: "User", email }, "dummy-token");
      setIsLoading(false);
      router.push("/");
    }, 1500);
  };
      ...
      >
        ...
        <form className="space-y-5" onSubmit={handleLogin}>
          ...
          <button 
            type="submit"
            disabled={isLoading}
            className="w-full bg-primary text-primary-foreground py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isLoading ? (
