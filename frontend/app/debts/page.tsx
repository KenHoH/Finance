        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-border bg-card p-6 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-4xl font-extrabold text-rose-500 tracking-tight">{formatCurrency(totalDebt).replace(',00', '')}</p>
            </div>
          </motion.div>
          ...
          {debtData.map((debt, idx) => (
            <motion.div key={debt.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + (0.1 * idx) }} className="rounded-3xl border border-border bg-card p-6 shadow-sm relative overflow-hidden group">
              ...
              <div className="space-y-2 mb-4">
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Budget</span><span className="font-semibold">{formatCurrency(debt.budgetAmount).replace(',00', '')}</span></div>
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Spent</span><span className="font-semibold">{formatCurrency(debt.totalSpent).replace(',00', '')}</span></div>
              </div>
              <div className="pt-4 border-t border-border flex justify-between items-center">
                <span className="text-sm font-bold text-muted-foreground">Debt Amount</span>
                <span className="text-xl font-extrabold text-rose-500">{formatCurrency(debt.debtAmount).replace(',00', '')}</span>
              </div>
            </motion.div>
          ))}
          {debtData.length === 0 && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-3xl border border-border bg-card p-6 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-4xl font-extrabold text-emerald-500 tracking-tight">+{formatCurrency(savingData.totalSavings).replace(',00', '')}</p>
            </div>
          </motion.div>
            ...
            {savingData.contributingBudgets.map((b, idx) => (
              <motion.div key={b.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + (0.1 * idx) }} className="flex items-center justify-between p-4 rounded-2xl border border-border bg-card shadow-sm">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Budget: {formatCurrency(b.budgetAmount).replace(',00', '')} | Spent: {formatCurrency(b.totalSpent).replace(',00', '')}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-bold text-muted-foreground uppercase mb-0.5">Saved</p>
                  <p className="text-xl font-extrabold text-emerald-500">+{formatCurrency(b.savedAmount).replace(',00', '')}</p>
                </div>
              </motion.div>
            ))}
          </div>
